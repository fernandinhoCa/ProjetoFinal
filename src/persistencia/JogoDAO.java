...
public class JogoDAO {
    ...
    public static boolean alterar(Jogo jogo){
        try{
            Connection conexao = Conexao.getConexao();
            String sql = "UPDATE jogo SET titulo=?, descricao=?, "
                    + "preco=?, numeroDias=?, "
                    + "categoria_id=?, memoria=?, tipo=? WHERE id=?";
            PreparedStatement ps = conexao.prepareStatement(sql);
            ps.setString(1, jogo.getTitulo());
            ps.setString(2, jogo.getDescricao());
            ps.setDouble(3, jogo.getPreco());
    ...
    public static boolean excluir(int id){
        ...
        try{
            Connection conexao = Conexao.getConexao();
            String sql = "select jogo.*, categoria.nome as categoria,"
                    + "categoria.tipo as tipo_categoria from jogo inner join"
                    + "categoria on jogo.categoria_id = categoria.id";
